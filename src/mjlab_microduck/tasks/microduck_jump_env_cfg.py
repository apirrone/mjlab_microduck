...
Episodic policy that crouches, jumps, then lands safely and returns to
standing.  The obs/action spaces are identical to the walking policy so the
two can be switched at runtime with a single key-press.

Phase encoding (in the command slot, 3-D):
    command = [cos(2π·phase), sin(2π·phase), 0]
    phase ∈ [0, 0.5]  → jump phase  (reward high trunk height)
    phase ∈ [0.5, 1]  → land phase  (reward returning to default pose)
...
def make_microduck_jump_env_cfg(play: bool = False) -> ManagerBasedRlEnvCfg:
    ...
    for name in [
        ...
        "foot_slip",
        "pose",
        # com_height_target would fight against jumping — remove if present
        "com_height_target",
    ]:
    ...
    # Land phase — neck/head: return to default (prevents flopping).
    ...
    cfg.rewards["joint_torques_l2"] = RewardTermCfg(
        func=microduck_mdp.joint_torques_l2, weight=-5e-4
    )
    ...
    # ── Command: cyclic phase encoding (2 s period) ───────────────────────────
